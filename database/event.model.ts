 ...
 * - Ensures time is stored in consistent 24-hour format (HH:MM)
 */
EventSchema.pre('save', function (next) {
  ...
  // Validate and normalize time format (HH:MM in 24-hour format)
  if (this.isModified('time')) {
    const timeRegex = /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/;
    if (!timeRegex.test(this.time)) {
      return next(
        new Error('Invalid time format. Please use HH:MM format (24-hour).')
      );
    }
